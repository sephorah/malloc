...
static void merge_next_block(size_t *current_block_header, size_t *current_block_footer)
{
    ...
    next_block_footer = (boundary_tag_t *)((size_t)next_block_header + get_size(next_block_header) - BOUNDARY_TAG_SIZE);
...
{
    boundary_tag_t *prev_block_footer = (boundary_tag_t *)((size_t)current_block - BOUNDARY_TAG_SIZE * 2);
...
{
    boundary_tag_t *current_block_header = (boundary_tag_t *)((size_t)current_block - BOUNDARY_TAG_SIZE);
    boundary_tag_t *current_block_footer = (boundary_tag_t *)((size_t)current_block + get_size(current_block_header) - BOUNDARY_TAG_SIZE * 2);

    *current_block_header = CLEAR_BIT(*current_block_header);
    *current_block_footer = CLEAR_BIT(*current_block_footer);
