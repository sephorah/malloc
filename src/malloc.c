...
void *find_free_block(size_t size)
{
    ...
    size_t count = 0;
    // fprintf(stderr, "Block tmp size %ld %lb\n", size_tmp, size_tmp);
...
// Must have free blocks that are recycled and reused for subsequent malloc
//  requests if possible
//  • Must have a malloc implementa0on that searches the heap for free blocks via
//  an implicit list (i.e. traverses block-by-block).

// //  a chaque malloc tu ajoutes un maillon à une liste avec sbrk et
// apres free tu flag le maillon à free
// allouer la taille + un header afin de garder une trace
// ce meme header que tu vas garder dans ta liste chainee